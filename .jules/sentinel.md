...
## 2026-03-31 - [Missing Global Security Headers]
**Vulnerability:** The backend lacked HTTP security headers (e.g., `X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`, `Strict-Transport-Security`). This could potentially expose the application to clickjacking, MIME-type sniffing, and other client-side attacks.
**Learning:** While modern frameworks provide some built-in protections, explicitly configuring security headers provides an essential layer of defense-in-depth, preventing browsers from making insecure assumptions.
**Prevention:** Always configure global security middleware (like `SecurityHeadersMiddleware`) in the backend framework to inject standard security headers on all responses, ensuring consistent baseline protection across the entire API.
