...
// Hoist static data outside the component to prevent unnecessary re-allocations
// and provide stable references for React.memo optimization.
...
const ImageCarousel = React.memo(({ images }) => {
  ...
  const handleStart = (clientX) => {
    ...
    if (containerRef.current) {
      containerRef.current.style.setProperty('--drag-offset', '0deg');
    }
  };
  ...
  const handleMove = (clientX) => {
    ...
    if (containerRef.current) {
      containerRef.current.style.setProperty('--drag-offset', `${offset / 10}deg`);
    }
  };
  ...
  const handleFrameMouseMove = (e, index) => {
    ...
    // ⚡ Bolt: Direct DOM manipulation for high-frequency tilt updates
    // This bypasses React re-renders for a smoother 60fps experience.
  ...
  const getImageStyles = (index) => {
    ...
    if (isThisHovered && !isDragging) {
      ...
      // rotateX is now handled by CSS variable --tilt-y
    }

    return {
      transform: `translateX(${translateX}%) scale(${scale}) rotateY(${rotateY}deg) rotateX(${isThisHovered && !isDragging ? 'var(--tilt-y, 0deg)' : '0deg'})`,
      zIndex: zIndex,
      opacity: opacity,
      boxShadow: boxShadow,
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            transform: `rotateY(var(--drag-offset, 0deg))`, // Visual Fan Tilt during drag
      ...
      <section className="relative pt-12 pb-8 px-4">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="mb-12 w-full overflow-visible">
            <ImageCarousel images={HERO_CAROUSEL_IMAGES} />
          </div>
