...
const ImageCarousel = ({ images }) => {
  ...
  // High-frequency values moved to CSS variables via direct DOM manipulation to prevent re-renders
  ...
  const handleStart = (clientX) => {
    ...
    if (containerRef.current) {
      containerRef.current.style.setProperty('--drag-offset', '0');
    }
  };
  ...
  const handleMove = (clientX) => {
    ...
    containerRef.current.style.setProperty('--drag-offset', offset.toString());
  };
  ...
  const getImageStyles = (index) => {
    ...
    const tiltValue = isThisHovered && !isDragging ? 'calc(var(--tilt-y, 0) * -8deg)' : '0deg';
    ...
    return {
      transform: `translateX(${translateX}%) scale(${scale}) rotateY(${rotateY}deg) rotateX(${tiltValue})`,
      zIndex: zIndex,
      opacity: opacity,
      boxShadow: boxShadow,
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            transform: 'rotateY(calc(var(--drag-offset, 0) * 0.1deg))', // Visual Fan Tilt during drag
...
// Hoist static data outside the component to prevent recreation on each render and reduce memory footprint.
