...
const ImageCarousel = ({ images }) => {
  ...
  const handleMove = (clientX) => {
    ...
    containerRef.current?.style.setProperty('--drag-offset', `${offset}`);
  };
  ...
  const getImageStyles = (index) => {
    ...
    if (isThisHovered && !isDragging) {
      ...
      // rotateX is handled via CSS variable --tilt-y for performance
    }

    return {
      transform: `translateX(${translateX}%) scale(${scale}) rotateY(${rotateY}deg) rotateX(calc(var(--tilt-y, 0) * -8deg))`,
      zIndex: zIndex,
      opacity: opacity,
      boxShadow: boxShadow,
