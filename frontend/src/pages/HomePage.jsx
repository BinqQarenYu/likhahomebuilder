...
// Static data hoisted for performance optimization (avoids re-creation on every render)
...
const ImageCarousel = React.memo(({ images }) => {
  ...
  const handleNext = React.useCallback((count = 1) => {
  ...
  const handlePrev = React.useCallback((count = 1) => {
    const moveCount = typeof count === 'number' ? count : 1;
    setTransitionDuration(400); // Snappier manual navigation
  ...
  const handleFrameMouseMove = (e, index) => {
    ...
    // Optimization: Update CSS variable directly to avoid React re-render on high-frequency mouse move
  ...
  const getImageStyles = (index) => {
    ...
    if (isThisHovered && !isDragging) {
      ...
      // rotateX uses CSS variable updated via direct DOM manipulation
      rotateX = 'calc(var(--tilt-y, 0) * -8deg)';
    }

    return {
      transform: `translateX(${translateX}%) scale(${scale}) rotateY(${rotateY}deg) rotateX(${rotateX})`,
      zIndex: zIndex,
      opacity: opacity,
      boxShadow: boxShadow,
