...
// Move static data outside the component to ensure stable object references
// and prevent redundant re-renders of memoized child components.
...
const ImageCarousel = React.memo(({ images }) => {
  ...
  const getImageStyles = (index) => {
    ...
    const tiltStr = isThisHovered && !isDragging ? 'rotateX(calc(var(--tilt-y, 0) * -8deg))' : 'rotateX(0deg)';
    ...
    return {
      transform: `translateX(${translateX}%) scale(${scale}) rotateY(${rotateY}deg) ${tiltStr}`,
      zIndex: zIndex,
      opacity: opacity,
      boxShadow: boxShadow,
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            transform: 'rotateY(calc(var(--drag-offset, 0) / 10 * 1deg))', // Visual Fan Tilt during drag
